/**
 * Genera el script de instalación que se incluirá en el paquete
 * @param {string} projectName - Nombre del proyecto
 * @returns {string} - Contenido del script de instalación
 */
function createInstallScript(projectName) {
...
if [ ! -d ".claudepack" ] || [ ! -f ".claudepack/manifest.json" ]; then
...
if ! command -v jq &> /dev/null; then
    ...
    PACKAGE_VERSION=$(jq -r '.claudepack_version' .claudepack/manifest.json 2>/dev/null || echo "desconocida")
    STRUCTURE_VERSION=$(jq -r '.structure_version' .claudepack/manifest.json 2>/dev/null || echo "desconocida")
    ORIGINAL_PROJECT_PATH=$(jq -r '.project_path' .claudepack/manifest.json 2>/dev/null || echo "desconocido")
    PROJECT_NAME=$(jq -r '.project_name' .claudepack/manifest.json 2>/dev/null || echo "${projectName}")
...
# Encontrar directorio .claude
CLAUDE_DB_PATH="\$HOME_DIR/.claude"

if [ ! -d "\$CLAUDE_DB_PATH" ]; then
    echo_warning "No se encontró el directorio .claude en \$CLAUDE_DB_PATH"
...
# Crear estructura de directorios en .claude
echo_status "Creando estructura en .claude..."
mkdir -p "\$CLAUDE_DB_PATH/projects/\$NORMALIZED_PATH"

# Verificar directorio de conversaciones
if [ ! -d ".claudepack/conversations" ]; then
    echo_warning "No se encontró el directorio '.claudepack/conversations'"
    echo_warning "No se instalarán conversaciones."
    INSTALL_CONVERSATIONS=false
else
    INSTALL_CONVERSATIONS=true
fi

# Verificar directorio de tareas
if [ ! -d ".claudepack/todos" ]; then
    echo_warning "No se encontró el directorio '.claudepack/todos'"
...
if [ ! -d ".claudepack/statsig" ]; then
    echo_warning "No se encontró el directorio '.claudepack/statsig'"
...
if [ "\$INSTALL_CONVERSATIONS" = true ]; then
    echo_status "Procesando conversaciones..."
    CONVERSATIONS_COUNT=0
    
    for CONV_FILE in .claudepack/conversations/*.jsonl; do
        if [ -f "\$CONV_FILE" ]; then
    ...
    echo_status "No se instalaron conversaciones."
fi
...
if [ "\$INSTALL_TODOS" = true ]; then
    ...
    for TODO_FILE in .claudepack/todos/*.json; do
        if [ -f "\$TODO_FILE" ]; then
...
if [ "\$INSTALL_STATSIG" = true ]; then
    ...
    for STATSIG_FILE in .claudepack/statsig/*; do
